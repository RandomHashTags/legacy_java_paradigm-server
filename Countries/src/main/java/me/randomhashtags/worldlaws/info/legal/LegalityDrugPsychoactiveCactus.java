...
public enum LegalityDrugPsychoactiveCactus implements LegalityDrug {
    ...
    private String getValue(String countryBackendID) {
        ...
        if(value.equals("null")) {
            WLLogger.log(Level.WARNING, "LegalityDrugPsychoactiveCactus - missing for country \"" + countryBackendID + "\"!");
        }
        return value;
    }
    ...
    private void refresh(CompletionHandler handler) {
        ...
        final String url = "https://en.wikipedia.org/wiki/Legal_status_of_psychoactive_cactus_by_country";
        final Document doc = getDocument(url);
        if(doc != null) {
            final Elements trs = doc.select("div.mw-parser-output table.wikitable").get(0).select("tbody tr");
            trs.remove(0);
            final EventSources sources = new EventSources(new EventSource("Wikipedia: Legal status of psychoactive cactus by country", url));
            for(Element element : trs) {
                final Elements tds = element.select("td");
                final String country = tds.get(0).text().toLowerCase().split("\\(")[0].replace(" ", "");
                final CountryInfoKey info = getInfoKey("Drug: Psychoactive Cacti", tds, sources, "Cultivation");
                countries.put(country, info.toString());
            }
            WLLogger.log(Level.INFO, "LegalityDrugPsychoactiveCactus - refreshed (took " + (System.currentTimeMillis()-started) + "ms)");
            handler.handle(null);
        }
    }
}
