...
public enum LegalityDrugPsilocybinMushrooms implements LegalityDrug {
    ...
    private String getValue(String countryBackendID) {
        ...
        if(value.equals("null")) {
            WLLogger.log(Level.WARNING, "LegalityDrugPsilocybinMushrooms - missing for country \"" + countryBackendID + "\"!");
        }
        return value;
    }
    ...
    private void refresh(CompletionHandler handler) {
        ...
        final String url = "https://en.wikipedia.org/wiki/Legal_status_of_psilocybin_mushrooms";
        final Document doc = getDocument(url);
        if(doc != null) {
            final Elements trs = doc.select("div.mw-parser-output table.wikitable").get(0).select("tbody tr");
            trs.remove(0);
            trs.remove(trs.size()-1);
            final EventSources sources = new EventSources(new EventSource("Wikipedia: Legal status of psilocybin mushrooms", url));
            for(Element element : trs) {
                final Elements tds = element.select("td");
                final String country = tds.get(0).text().toLowerCase().split("\\[")[0].replace(" ", "");
                final CountryInfoKey info = getInfoKey("Drug: Psilocybin Mushrooms", tds, sources, "Cultivation");
                countries.put(country, info.toString());
            }
            WLLogger.log(Level.INFO, "LegalityDrugPsilocybinMushrooms - refreshed (took " + (System.currentTimeMillis()-started) + "ms)");
            handler.handle(null);
        }
    }
}
