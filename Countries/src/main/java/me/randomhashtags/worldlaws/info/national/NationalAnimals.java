...
public enum NationalAnimals implements CountryNationalService {
    ...
    public SovereignStateInfo getInfo() {
        return SovereignStateInfo.NATIONAL_ANIMALS;
    }

    @Override
    public JSONObjectTranslatable loadData() {
        final Elements elements = getDocumentElements(Folder.COUNTRIES_NATIONAL, "https://en.wikipedia.org/wiki/List_of_national_animals", "div.mw-parser-output table.wikitable", 0).select("tbody tr");
        return getNationalArrayJSON(elements, "National Animal");
    }
}
