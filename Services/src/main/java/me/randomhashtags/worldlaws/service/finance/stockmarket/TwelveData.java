...
public enum TwelveData implements StockService {
                    ...
                    public void handleJSONObject(JSONObject json) {
                        ...
                        json.keySet().parallelStream().forEach(symbol -> {
                            ...
                            if(completed.addAndGet(1) == max) {
                                getQuotes(version, quoteURL, symbols, max, prices, handler);
                            }
                        });
                    }
                });
    ...
    private void getQuotes(APIVersion version, String quoteURL, HashSet<String> symbols, int max, HashMap<String, Float> prices, CompletionHandler handler) {
        ...
        requestJSONObject(url, RequestMethod.GET, new CompletionHandler() {
            ...
            public void handleJSONObject(JSONObject json) {
                ...
                StreamSupport.stream(valuesArray.spliterator(), true).forEach(obj -> {
                    ...
                    if(completed.addAndGet(1) == outputsize) {
                        final StringBuilder builder = new StringBuilder();
                        boolean isFirst = true;
